...
function _debian() {
    sudo -S apt install fish terminator sqlite3 sqlitebrowser \
        python3 python3-pip git nodejs npm tlp tlp-rdw gnome-tweak-tool ubuntu-restricted-extras neovim vifm
}
...
function _fedora() {
    sudo dnf update &&
        ...
        sudo dnf install git nodejs npm fish vlc sqlite sqlitebrowser tlp tlp-rdw golang &&
